...
export default async function _getVendors(mssqlConfig, vendorFilters) {
    const pool = await connect(mssqlConfig);
    const { parameters, whereClause } = buildVendorFiltersWhereClause(vendorFilters ?? {});
    const request = pool.request();
    for (const [parameterName, parameterValue] of Object.entries(parameters)) {
        request.input(parameterName, parameterValue);
    }
    const vendorResult = await request.query(`SELECT
...
function buildVendorFiltersWhereClause(vendorFilters) {
    const whereClauses = [];
    const parameters = {};
    if (vendorFilters.vendorClassIds !== undefined &&
        vendorFilters.vendorClassIds.length > 0) {
        whereClauses.push(`v.VNDCLSID IN (${vendorFilters.vendorClassIds
            .map((_, index) => `@vendorClassId${index}`)
            .join(', ')})`);
        for (const [index, vendorClassId] of vendorFilters.vendorClassIds.entries()) {
            parameters[`vendorClassId${index}`] = vendorClassId;
        }
    }
    if (vendorFilters.notVendorClassIds !== undefined &&
        vendorFilters.notVendorClassIds.length > 0) {
        whereClauses.push(`v.VNDCLSID NOT IN (${vendorFilters.notVendorClassIds
            .map((_, index) => `@notVendorClassId${index}`)
            .join(', ')})`);
        for (const [index, notVendorClassId] of vendorFilters.notVendorClassIds.entries()) {
    ...
    if (vendorFilters.vendorNameContains !== undefined) {
        for (const [index, vendorNameContains] of vendorFilters.vendorNameContains.entries()) {
            whereClauses.push(`(v.VENDNAME LIKE '%' + @vendorNameContains${index} + '%' OR
    ...
    if (vendorFilters.vendorNameDoesNotContain !== undefined &&
        vendorFilters.vendorNameDoesNotContain.length > 0) {
        for (const [index, vendorNameDoesNotContain] of vendorFilters.vendorNameDoesNotContain.entries()) {
            whereClauses.push(`(v.VENDNAME NOT LIKE '%' + @vendorNameDoesNotContain${index} + '%' AND
